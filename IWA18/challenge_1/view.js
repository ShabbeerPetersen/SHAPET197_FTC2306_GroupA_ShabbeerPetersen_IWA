...
/**
 * Takes any order as an object literal (as saved in state)
 * and converts it a
 * HTML element that can be appended to the DOM. Creating
 * order elements
 * individually prevents the JavaScript having to re-render
 * the entire DOM every
...
/**
 * Since the tables in use, and their identification can be
 * configured before
 * the start of the app (in data.js), the actual options
 * returned should be
 * dynamically added to the respective "<select>" elements in
 * the HTML after
 * JavaScript loads. This function executes the logic thar
 * reads the current
...
/**
 * An object literal that contains references to all the
 * HTML elements
 * referenced through the operation of the app either upon
 * initialisation or
 * while its running (via event listeners). This ensure that
 * all UI elements can
 * be accessed and seen in a structured manner in a single
 * data structure.
 *
 * Note that the "column" and "area" properties created as
 * empty and then added
...
/**
 * Maps over all columns in the HTML and removes any dragging
 * hover effects
 * except for the current column that is being dragged over
 * (if at all). If the
 * "over" value is not specified, then all columns will be
 * cleared of any hover
 * effects.
 *
...
/**
 * Takes a specific order HTML and clones it into memory.
 * The original HTML
 * element is then removed from the DOM, while the cloned
 * duplicate is added to
 * the bottom of the column that is specified.
 *
 * @param {string} id - The "id" value of a specific order
 * object. Note that
 * only the "id" value is used, not the entire object.
 *
 * @param {string} newColumn - The name of the column that
 * the order should be
 * moved to. This should coincide with one of the values
...
/**
 * Starts the app focused on the "add order" button. This
 * means that users can
 * immediately started adding an order by pressing the enter
 * or spacebar.
 */
